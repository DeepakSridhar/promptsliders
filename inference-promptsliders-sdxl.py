...
def encode_prompts_xl(
    ...
    return torch.concat(text_embeds_list, dim=-1), pooled_text_embeds.view(1, -1)
...
def call(
        ...
        prompt_embeds, pooled_prompt_embeds = encode_prompts_xl([pipe.tokenizer, pipe.tokenizer_2], [pipe.text_encoder, pipe.text_encoder_2], [prompt], sc=scale)

        prompt_embeds2, pooled_prompt_embeds2 = encode_prompts_xl([pipe.tokenizer, pipe.tokenizer_2], [pipe.text_encoder, pipe.text_encoder_2], [prompt], sc=0.0)
        ...
        if do_classifier_free_guidance:
            ...
            prompt_embeds2 = torch.cat([negative_prompt_embeds, prompt_embeds2], dim=0)
            add_text_embeds = torch.cat([negative_pooled_prompt_embeds, add_text_embeds], dim=0)
            add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)

        prompt_embeds = prompt_embeds.to(device)
        prompt_embeds2 = prompt_embeds2.to(device)
...
new_token = f"<{cname}>"
learned_embeds_path = f"output/{concept}-slider_prompt/learned_embeds.safetensors" #path to the trained sliders
device = 'cuda:0'
...
if new_token not in pipe.tokenizer.get_vocab():
    # Add the token to the tokenizer
    pipe.tokenizer.add_tokens([new_token])
...
with torch.no_grad():
    pipe.text_encoder.get_input_embeddings().weight.data[new_token_id] = new_token_embed.clone()

prompts = [
    # f'Image of a person, realistic, {new_token}, 8k',
    f'A photo of a girl, {new_token}',
#     'A realistic photograph of a person, bokeh, blurred background, {new_token}, 8k',
    f'Professional headshot of a person, {new_token}',
]
...
for prompt in prompts:
    for _ in range(num_images_per_prompt):
        seed = random.randint(0,2**15)  #22373
        print(prompt, seed)
        ...
        plt.suptitle(f'{concept}', fontsize=20)
        # plt.tight_layout()
        plt.show()
        plt.savefig(f'sdxl-{prompt}-{seed}.png')
